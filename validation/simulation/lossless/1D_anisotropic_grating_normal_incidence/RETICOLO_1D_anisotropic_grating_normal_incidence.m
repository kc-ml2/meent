% Complex r/t amplitude coefficients and single-wavelength spatial fields for 1D_anisotropic_grating_normal_incidence.
%
...
if QUICK
    ...
    % Offset the grid from exact Rayleigh/internal modal cutoffs, where kz = 0 and
    % flux-normalized modal amplitudes are singular in both solvers.
...
% placeholder and res1 swaps in the diagonal tensor through parm.res1.change_index.
n_x = 1.5;  n_y = 2;  n_z = 2.5;
...
for ip = 1:NP
    for id = 1:ND
        out_files{ip, id} = sprintf('RETICOLO_1D_anisotropic_grating_normal_incidence_%s_%s.txt', POLS{ip}, DIRS{id});
...
if RUN_FIELD_PROFILE
    ...
    reticolo_export_field('1D_anisotropic_grating_normal_incidence', FIELD_WAVELENGTH_M, period, textures, ...
