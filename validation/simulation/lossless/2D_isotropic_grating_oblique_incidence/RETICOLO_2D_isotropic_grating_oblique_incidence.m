% Complex r/t amplitude coefficients and single-wavelength spatial fields for 2D_isotropic_grating_oblique_incidence.
%
...
% Centred rectangular pillar on a uniform slab.
textures = {n_incident_medium, ...
            {n_bg, [period(1)/2, period(2)/2, grating_dx, grating_dy, n_inc, 1]}, ...
            n_inc, ...
...
for ip = 1:NP
    for id = 1:ND
        out_files{ip, id} = sprintf('RETICOLO_2D_isotropic_grating_oblique_incidence_%s_%s.txt', POLS{ip}, DIRS{id});
...
if RUN_FIELD_PROFILE
    ...
    reticolo_export_field('2D_isotropic_grating_oblique_incidence', FIELD_WAVELENGTH_M, period, textures, ...
