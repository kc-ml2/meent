% Complex r/t amplitude coefficients and single-wavelength spatial fields for 2D_anisotropic_grating_oblique_incidence.
%
...
n_x = 1.5 + 0.05i;  n_y = 2 + 0.1i;  n_z = 2.5 + 0.15i;
n_placeholder = 89.99999;
n_bg = 1.0;
grating_dx = 500e-9;
grating_dy = 400e-9;

textures = {n_incident_medium, ...
            {n_bg, [period(1)/2, period(2)/2, grating_dx, grating_dy, n_placeholder, 1]}, ...
            n_placeholder, ...
            n_transmitted_medium};
...
for ip = 1:NP
    for id = 1:ND
        out_files{ip, id} = sprintf('RETICOLO_2D_anisotropic_grating_oblique_incidence_%s_%s.txt', POLS{ip}, DIRS{id});
...
if RUN_FIELD_PROFILE
    ...
    reticolo_export_field('2D_anisotropic_grating_oblique_incidence', FIELD_WAVELENGTH_M, period, textures, ...
