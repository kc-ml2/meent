...
class LpLoss(object):
    ...
    def rel(self, x, y):
        ...
        # (anthony) self.d+1 for [real, imaginary] -> [real + imaginary]
        diff = torch.norm(torch.flatten(x, start_dim=-(self.d+1)) - torch.flatten(y, start_dim=-(self.d+1)), \
                          p=self.p, dim=-1, keepdim=False)
        ynorm = torch.norm(torch.flatten(y, start_dim=-(self.d+1)), p=self.p, dim=-1, keepdim=False)
...
class H1Loss(object):
    ...
    def rel(self, x, y, h=None):
        ...
        # (anthony) flatten for [real, imaginary] -> [real + imaginary]
        diff = torch.norm((dict_x[0] - dict_y[0]).flatten(1), p=2, dim=-1, keepdim=False)**2
        ynorm = torch.norm(dict_y[0].flatten(1), p=2, dim=-1, keepdim=False)**2

        for j in range(1, self.d + 1):
            diff += torch.norm((dict_x[j] - dict_y[j]).flatten(1), p=2, dim=-1, keepdim=False)**2
            ynorm += torch.norm(dict_y[j].flatten(1), p=2, dim=-1, keepdim=False)**2
