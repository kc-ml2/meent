...
def main(fabric: Fabric, cfg: Dict[str, Any]):
    # torch.manual_seed(cfg.env.seed) # this will affect torch.distribution
        ...
        with torch.inference_mode():
            ...
            with timer("Time/env_interaction_time", SumMetric, sync_on_compute=False):
                ...
                if (
                    ...
                    #######################
                    # with torch.no_grad():
                    #     _img_stoch, _recur = world_model.rssm.imagination(player.stochastic_state, player.recurrent_state, actions[0])
                    #     _img_lat = torch.cat((_img_stoch.view(1, 8, -1), _recur), -1)
                    #     _rec_obs = world_model.observation_model(_img_lat)
                    #     _l.append(_rec_obs)#.cpu().numpy())
                    ########################
...
################################
                max_idx = np.argmax(infos['max_eff'])
                max_eff = float(infos['max_eff'].max())
                aggregator.update("Env/max_eff", max_eff)
                # if max_eff > 0.7:
                #     with open(f'/data1/neurips2024-meent/{max_idx}_{max_eff}.pkl', 'wb') as fp:
                #         pickle.dump(dict(
                #             struct_traj = envs.get_attr('struct_traj')[max_idx],
                #             field_traj = envs.get_attr('field_traj')[max_idx],
                #             action_traj = envs.get_attr('action_traj')[max_idx],
                #             # rec_traj = _l
                #             # stoch_traj = player.stochastic_states,
                #             # recur_traj = player.recurrent_states,
                #         ), fp)
                ...
                #     }
                #     ckpt_path = f"/data1/neurips2024-meent/{max_idx}_{max_eff}_ckpt_{policy_step}_{fabric.global_rank}.ckpt"
                #     fabric.call(
