...
def transfer_1d_1(pol, kx, n_top, n_bot, device=torch.device('cpu'), type_complex=torch.complex128):
    ...
    kz_top[torch.where(abs(kx)>n_top)] = -1j*(kx[torch.where(abs(kx)>n_top)] ** 2 - n_top ** 2) ** 0.5
    kz_bot[torch.where(abs(kx)>n_bot)] = -1j*(kx[torch.where(abs(kx)>n_bot)] ** 2 - n_bot ** 2) ** 0.5
...
def transfer_1d_conical_1(kx, ky, n_top, n_bot, device='cpu', type_complex=torch.complex128):
    ...
    kz_top[torch.where(abs(kx**2+ky.reshape((-1,1))**2)>abs(n_top.real**2))] = (-1j*((kx**2+ky.reshape((-1,1))**2)
                       [torch.where(abs(kx**2+ky.reshape((-1,1))**2)>abs(n_top.real**2))] - (n_top ** 2)) ** 0.5)
    kz_bot[torch.where(abs(kx**2+ky.reshape((-1,1))**2)>abs(n_bot.real**2))] = (-1j*((kx**2+ky.reshape((-1,1))**2)
                       [torch.where(abs(kx**2+ky.reshape((-1,1))**2)>abs(n_bot.real**2))] - (n_bot ** 2)) ** 0.5)
...
## transfer matrix for anisotropic cases
def transfer_1d_1_aniso(pol, kx, n_top, n_bot, device=torch.device('cpu'), type_complex=torch.complex128):
    ff_x = len(kx)
...
def transfer_1d_2_aniso(pol, kx, epx_conv, epy_conv, epz_conv_i, device=torch.device('cpu'), type_complex=torch.complex128,
                  perturbation=1E-20, use_pinv=False):
...
def transfer_1d_3_aniso(k0, W, V, q, d, F, G, T, device=torch.device('cpu'), type_complex=torch.complex128, use_pinv=False):
    ff_x = len(q)
...
def transfer_1d_4_aniso(pol, ff_x, F, G, T, kz_top, kz_bot, theta, n_top, n_bot, device=torch.device('cpu'),
                  type_complex=torch.complex128, use_pinv=False):
...
def transfer_1d_conical_1_aniso(kx, ky, n_top, n_bot, device='cpu', type_complex=torch.complex128):
    ff_x = len(kx)
    ff_y = len(ky)
    ff_xy = ff_x * ff_y
...
# def transfer_1d_conical_2(k0, Kx, ky, E_conv, E_i, o_E_conv_i, ff, d, varphi, big_F, big_G, big_T,
#                           device='cpu', type_complex=torch.complex128, perturbation=1E-10):
def transfer_1d_conical_2_aniso(kx, ky, epx_conv, epy_conv, epz_conv_i, device='cpu', type_complex=torch.complex128,
                          perturbation=1E-20, use_pinv=False):
...
# def transfer_1d_conical_3(big_F, big_G, big_T, Z_I, Y_I, psi, theta, ff, delta_i0, k_I_z, k0, n_I, n_II, k_II_z,
#                           device='cpu', type_complex=torch.complex128):
def transfer_1d_conical_3_aniso(k0, W, V, q, d, varphi, big_F, big_G, big_T, device='cpu', type_complex=torch.complex128,
                          use_pinv=False):
...
def transfer_1d_conical_4_aniso(ff_x, ff_y, big_F, big_G, big_T, kz_top, kz_bot, psi, theta, n_top, n_bot, device='cpu',
                          type_complex=torch.complex128, use_pinv=False):
...
def transfer_2d_1_aniso(kx, ky, n_top, n_bot, device=torch.device('cpu'), type_complex=torch.complex128):
    ff_x = len(kx)
    ff_y = len(ky)
    ff_xy = ff_x * ff_y
...
def transfer_2d_2_aniso(kx, ky, epx_conv, epy_conv, epz_conv_i, device=torch.device('cpu'), type_complex=torch.complex128,
                   perturbation=1E-20, use_pinv=False):
...
def transfer_2d_3_aniso(k0, W, V, q, d, varphi, big_F, big_G, big_T, device=torch.device('cpu'),
                  type_complex=torch.complex128, use_pinv=False):
    ff_xy = len(q)//2
...
def transfer_2d_4_aniso(ff_x, ff_y, big_F, big_G, big_T, kz_top, kz_bot, psi, theta, n_top, n_bot,
                  device=torch.device('cpu'), type_complex=torch.complex128, use_pinv=False):
