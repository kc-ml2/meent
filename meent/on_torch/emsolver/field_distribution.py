...
def field_dist_2d(wavelength, kx, ky, T1, layer_info_list, period,
    ...
    # ky = k0 * (n_top * torch.sin(theta) * torch.sin(phi) + fourier_indices_y * (
    #         wavelength / period[1])).type(type_complex)
    #
    # Kx = torch.diag(torch.tile(kx, (ff_y, )).flatten()) / k0
    # Ky = torch.diag(torch.tile(ky.reshape((-1, 1)), (ff_x, )).flatten()) / k0
    #
