...
def to_conv_mat_vector(ucell_info_list, fto_x, fto_y, device=torch.device('cpu'),
    ...
    #                                       fto_x, fto_y, device=device, type_complex=type_complex)
...
# def fs_rectangle(cx, cy, lx, ly, pmtvy, fto, period):
#     Px, Py = period
#     fourier_x, fourier_y = fto
...
def to_conv_mat_raster_continuous(ucell, fto_x, fto_y, device=torch.device('cpu'),
    ...
    #
    #         f_coeffs = fft_piecewise_constant(cell, x, y, fto_x, fto_y, device=device, type_complex=type_complex)
    #         o_f_coeffs = fft_piecewise_constant(1 / cell, x, y, fto_x, fto_y, device=device, type_complex=type_complex)
    #
    #         center = torch.tensor(f_coeffs.shape, device=device) // 2
    ...
    #         cell, x, y = cell_compression(layer, device=device, type_complex=type_complex)
    #
    #         f_coeffs = fft_piecewise_constant(cell, x, y, fto_x, fto_y, device=device,
    #                                           type_complex=type_complex)
    #         o_f_coeffs = fft_piecewise_constant(1 / cell, x, y, fto_x, fto_y, device=device,
    #                                             type_complex=type_complex)
    #
    #         center = torch.tensor(f_coeffs.shape, device=device) // 2
    #
...
#TODO: cleaning
def to_conv_mat_raster_discrete_(ucell, fourier_order_x, fourier_order_y, device=torch.device('cpu'), type_complex=torch.complex128,
                                improve_dft=True):
    ucell_pmt = ucell ** 2
