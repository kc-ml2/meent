...
def sweep_wavelength(wls, mode=0, *args, **kwargs):
    ...
    spectrum_r, spectrum_t = init_spectrum_array(solver.grating_type, wls, solver.fourier_order)

    for i, wavelength in enumerate(wls):

        solver.wls = np.array([wavelength])
...
def init_spectrum_array(grating_type, wls, fourier_order):
    if grating_type in (0, 1):
        spectrum_r = np.zeros((len(wls), 2 * fourier_order + 1))
        spectrum_t = np.zeros((len(wls), 2 * fourier_order + 1))
    elif grating_type == 2:
        spectrum_r = np.zeros((len(wls), 2 * fourier_order + 1, 2 * fourier_order + 1))
        spectrum_t = np.zeros((len(wls), 2 * fourier_order + 1, 2 * fourier_order + 1))
    else:
        raise ValueError
    return spectrum_r, spectrum_t
